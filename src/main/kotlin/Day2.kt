...
val test_input = """1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc"""

val puzzle_input = """3-4 j: tjjj
...
fun main() {
    day2(test_input, false)
    day2(puzzle_input, false)

    day2(test_input, true)
    day2(puzzle_input, true)
}

fun day2(input: String, part2: Boolean) {
    // Split out each line and check to see if it is individually correct, then sum up the correct ones
    var sum = 0
    for (line in input.split("\n")) {
        sum += isCorrect(line, part2)
    }

    println(sum)
}

fun isCorrect(line: String, part2: Boolean):Int {
    val lineParts = line.split(" ")
    val numbers = lineParts[0].split("-")
    val minNumber = numbers[0].toInt()
    val maxNumber = numbers[1].toInt()

    val letter = lineParts[1].split(":")[0].toCharArray()[0]

    return if (part2) passwordCorrectnessCheckPart2(lineParts[2], letter, minNumber, maxNumber)
    else passwordCorrectnessCheckPart1(lineParts[2], letter, minNumber, maxNumber)
}

fun passwordCorrectnessCheckPart2(password: String, letter: Char, firstNumber: Int, secondNumber: Int): Int {
    return if (passwordCorrectnessCheckPart2Worker(password, letter, firstNumber, secondNumber)) 1
    else 0
}

fun passwordCorrectnessCheckPart2Worker(password: String, letter: Char, firstNumber: Int, secondNumber: Int): Boolean {
    return (password[firstNumber - 1] == letter) xor (password[secondNumber - 1] == letter)
}

fun passwordCorrectnessCheckPart1(password: String, letter: Char, minNumber: Int, maxNumber: Int):Int {
    val count = password.filter { it == letter }.count()
    return if (count in minNumber..maxNumber) 1
    else 0
}