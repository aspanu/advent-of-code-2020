...
/**
 * Created by aspanu on 2020-12-03.
 */

val test_input = """ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
val puzzle_input = """iyr:2013 hcl:#ceb3a1
hgt:151cm eyr:2030
byr:1943 ecl:grn
...
val test_input_invalid = """eyr:1972 cid:100
...
val test_input_valid = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
fun main() {
    assert(day4(test_input, false) == 2)
    println(day4(puzzle_input, false))

    println(day4(test_input_invalid, true))
    println(day4(test_input_valid, true))
    println(day4(puzzle_input, true))
}

fun day4(input: String, part2: Boolean): Int {
    // Take the input and split it up into passports (dividing by `\n\n`)
    // Then take the strings that are remaining and find all of the keys that exist and check to see if they are valid

    val passports = input.split("\n\n")
    var sum = 0
    if (part2) {
        passports.forEach { sum += passportValidCheckPart2(it) }
    } else {
        passports.forEach { sum += passportValidCheckPart1(it) }
    }

    return sum
}

fun passportValidCheckPart2(passportString: String): Int {
    // Receive a full passport string, which may have new lines in it, so let's split it all out
    val passportItems = mutableListOf<String>()
    passportString.split("\n").forEach { passportItems.addAll(it.split(" ")) }

    val passportInfo = mutableMapOf<String, String>()

    passportItems.forEach { passportInfo[it.split(":")[0]] = it.split(":")[1] }

    return if (passportValid(passportInfo)) 1
    else 0
}

fun passportValid(passportInfo: Map<String, String>): Boolean {
    if (passportInfo.keys.size < 7 || (passportInfo.keys.size == 7 && passportInfo.keys.contains("cid"))) return false

    if (passportInfo["byr"]!!.length != 4 || !insideRange(passportInfo["byr"]!!.toInt(), 1920, 2002)) return false
    if (passportInfo["iyr"]!!.length != 4 || !insideRange(passportInfo["iyr"]!!.toInt(), 2010, 2020)) return false
    if (passportInfo["eyr"]!!.length != 4 || !insideRange(passportInfo["eyr"]!!.toInt(), 2020, 2030)) return false
    if (!heightPasses(passportInfo["hgt"]!!)) return false
    if (!hairColourPasses(passportInfo["hcl"]!!)) return false
    if (!eyeColourPasses(passportInfo["ecl"]!!)) return false
    if (!passportIdPasses(passportInfo["pid"]!!)) return false

    return true
}

fun passportIdPasses(passportIdString: String): Boolean {
    return passportIdString.length == 9 && passportIdString.toLongOrNull()?. let {true} ?: false
}

fun eyeColourPasses(eyeClrString: String): Boolean {
    val eyeColourSet = setOf("amb" ,"blu" ,"brn" ,"gry" ,"grn" ,"hzl" ,"oth")
    return eyeColourSet.contains(eyeClrString)
}

fun hairColourPasses(hairClrString: String): Boolean {
    val regex = "#[0-9a-f]{6}".toRegex()

    return regex.matches(hairClrString)
}

fun heightPasses(heightString: String): Boolean {
    return if (heightString.contains("cm")) {
        heightString.split("cm")[0].toInt() in 150..193
    } else {
        heightString.split("in")[0].toInt() in 59..76
    }
}

fun insideRange(number: Int, bottom: Int, top: Int): Boolean {
    return number in bottom..top
}

fun passportValidCheckPart1(passportString: String): Int {
    // Receive a full passport string, which may have new lines in it, so let's split it all out
    val passportItems = mutableListOf<String>()
    passportString.split("\n").forEach { passportItems.addAll(it.split(" ")) }
    ...
    passportItems.forEach { keys.add(it.split(":")[0]) }

    return if (keys.size == 8 || (keys.size == 7 && !keys.contains("cid"))) 1
    else 0
}
