...
/**
 * Created by aspanu on 2020-12-06.
 */
...
val puzzle_input = """vkplsqwiftuazyje
mokluxwbsfhgc
...
fun main() {
    println(day6(test_input))
    println(day6(puzzle_input))
    println(day6(puzzle_input, true))
    println(day6(puzzle_input, true))
}

fun day6(input: String, partTwo: Boolean = false): Int {
    // Take each block and find the number of unique letters in it
    val blocks = input.split("\n\n")
    var sum = 0

    blocks.forEach { sum += if (partTwo) getIntersectionOfLetters(it) else getCountOfLetters(it) }

    return sum
}

fun getIntersectionOfLetters(block: String): Int {
    //Given a block, take each line and find the intersection (i.e. where everyone wrote yes
    ...
    block.split("\n").forEach { str ->
        totalNumber += 1
        str.forEach {
            charFreq[it] = (charFreq[it] ?: 0) + 1
        }
    }

    println("This")

    return charFreq.filter { it.value == totalNumber }.size
}

fun getCountOfLetters(block: String): Int {
    val charSet = mutableSetOf<Char>()

    block.split("\n").forEach { str -> str.forEach { charSet.add(it) } }
